...
public MsgRespose loginSurvey(long code){
	try {
		...
		if(surveyVM.getStatus().equals(Survey.STATUS_BEGIN)){
			...
			return MsgRespose.error("课调状态不合法:" +surveyVM.getStatus());
		}
...
public MsgRespose checkSurvey( long id,int status){
	try {
		...
		return MsgRespose.success("message", null);
	} catch (Exception e) {
		e.printStackTrace();
		return MsgRespose.error(e.getMessage());
	}
...
public MsgRespose findSurveyVMByStatus(String status){
	try {
		List<SurveyVM> list=surveyService.findByStatus(status);
		
		return MsgRespose.success("成功关闭", list);
	} catch (Exception e) {
		e.printStackTrace();
		return MsgRespose.error(e.getMessage());
	}
...
public MsgRespose stopSurvey(long id){
	try {
		Survey survey=surveyService.findSurveyById(id);
		survey.setStatus(Survey.STATUS_NO_CHECK);
		return MsgRespose.success("成功关闭", null);
	} catch (Exception e) {
		e.printStackTrace();
		return MsgRespose.error(e.getMessage());
	}
...
public MsgRespose startSurvey(long id){
	try {
		Survey survey=surveyService.findSurveyById(id);
		//当课调状态未开启的时候才能开启一个课调
		...
		survey.setCode(code);
		surveyService.saveOrUpdateSurvey(survey);
		return MsgRespose.success("开启成功", null);
		}else{
			return MsgRespose.success("课调状态不合法"+survey.getStatus(), null);
		}
...
@PostMapping(value="saveOrUpadteSurvey")
	public MsgRespose saveOrUpdateSurvey(Survey survey){
	try {
		surveyService.saveOrUpdateSurvey(survey);
...
@GetMapping(value="toCheckSurvey")
	public MsgRespose toCheckSurvey(long id, int singleTotal, String selectStr){
		try {
			//1、根据id查询课调信息
			SurveyVM surveyVM=surveyService.findById(id);
			//2、根据id查询课调中所有的问卷信息
